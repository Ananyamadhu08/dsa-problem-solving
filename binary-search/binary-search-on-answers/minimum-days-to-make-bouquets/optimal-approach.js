...
function minimumDaysToMakeBouquets(arr, bouquets, flowers) {
  ...
  const low = minAndMaxElements[0];
  const high = minAndMaxElements[1];

  while (low <= high) {
    let mid = Math.floor((low + high) / 2);

    if (checkPossibleBouquets(arr, mid, bouquets, flowers)) {
      high = mid - 1;
    } else {
      low = mid + 1;
    }
...
// Time complexity: O(log(maxElement - (minElement + 1)) * N)
