...
function wordSearch(board, word) {
  ...
  function search(row, col, index) {
    ...
    // ❌ If out of bounds or character mismatch, stop here
    if (
      row < 0 ||
      row >= rows ||
      col < 0 ||
    ...
    const exists =
      search(row + 1, col, index + 1) || // go down
      search(row - 1, col, index + 1) || // go up
      search(row, col + 1, index + 1) || // go right
      search(row, col - 1, index + 1); // go left
...
// - In-place marking avoids extra visited structures (no Set).
